...
function renderProjectCard(project, index) {
    ...
    const tagsHtml = project.tags.map(tag => `<span class="inline-block bg-blue-50 text-blue-700 text-xs font-medium tracking-wide px-2.5 py-0.5 rounded-md border border-blue-100 mr-2 mb-2 transition duration-200 ease-in-out hover:bg-blue-100">${tag}</span>`).join('');
    const highlightsHtml = project.highlights && project.highlights.length > 0 ?
        `<div class="mt-4 pt-4 border-t border-gray-100">
            ...
            <ul class="space-y-1.5 text-gray-700 text-sm">
                ${project.highlights.map(highlight => `<li class="flex items-start">
